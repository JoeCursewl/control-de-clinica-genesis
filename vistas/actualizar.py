...
class Actualizar(ctk.CTkFrame):
    ...
    def actualizar_window(self):
        self.app_actualizar = ctk.CTkToplevel(self)
        w, h = self.app_actualizar.winfo_screenwidth(), self.app_actualizar.winfo_screenheight()
        x, y = (w / 2) - 475, (h / 2) - 370
        self.app_actualizar.geometry(f"950x660+{int(x)}+{int(y)}")
        self.app_actualizar.grab_set()
        self.app_actualizar.title("Administración | Actualizar historia clínica")
        self.app_actualizar.resizable(0,0)
        
        self.letra_label_divisor = ('Century Gothic', 16)
        self.letra_label = ('Century Gothic', 15)
        
        
        frame_top = ctk.CTkFrame(self.app_actualizar, width=900, height=50)
        frame_top.place(x=475, y=10, anchor="n")
        
        label_divisor = ctk.CTkLabel(frame_top, text="Administración | Actualizar historia clínica                                                                                                                                             ", font=self.letra_label_divisor)
        label_divisor.pack(padx=20, pady=10)
        
        #######################################################################
        
        frame_datos_paciente = ctk.CTkFrame(self.app_actualizar, width=475, height=500)
        ...
        frame_datos_paciente2 = ctk.CTkFrame(self.app_actualizar, width=475, height=500)
        ...
        self.boton_registrar = ctk.CTkButton(self.app_actualizar, text="Actualizar Historia clínica", command=self.actualizar_historia_clinica)
        self.boton_registrar.place(x=580, y=618, anchor="center")
        
        self.boton_registrar = ctk.CTkButton(self.app_actualizar, text="Consultar Historia clínica", command=self.consultar_historia_clinica)
                ...
                if tension.isdigit():
                    if talla.isdigit():
                        if peso.isdigit():
                            ...
                            if dato is not None:
                                self.db.actualizar_historia_clinica(id_historia=id_numero_hist, peso=peso, tension=tension, talla=talla, exam_fisico=exam_fisico, sintomas=sintomas, diagnostico=diagnostico, tratamiento=tratamiento, antecedentes=antecedentes, ante_family=ante_family)
                                CTkMessagebox(title="Clínica | Actualizada correctamente",
                                message="La historia ha sido actualizada correctamente!",
                                icon="check",
