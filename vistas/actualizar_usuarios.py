...
class Actualizar_usuarios(ctk.CTkFrame):
    ...
    def actualizar_usuarios_window(self):
        self.app_actualizar_usuarios = ctk.CTkToplevel(self)
        w, h = self.app_actualizar_usuarios.winfo_screenwidth(), self.app_actualizar_usuarios.winfo_screenheight()
        x, y = (w / 2) - 475, (h / 2) - 370
        self.app_actualizar_usuarios.geometry(f"950x660+{int(x)}+{int(y)}")
        self.app_actualizar_usuarios.grab_set()
        self.app_actualizar_usuarios.title("Administración | Registrar Usuarios")
        self.app_actualizar_usuarios.resizable(0,0)
        ...
        frame_top = ctk.CTkFrame(self.app_actualizar_usuarios, width=900, height=50)
        ...
        label_user_admin = ctk.CTkLabel(self.app_actualizar_usuarios, text="Usuario admin", font=self.letra_label)
        label_user_admin.place(x=80, y=150, anchor="w")
        
        self.entry_user_admin = ctk.CTkEntry(self.app_actualizar_usuarios, width=170)
        self.entry_user_admin.place(x=80, y=190, anchor="w")
        
        #####################################################
        
        label_password_admin = ctk.CTkLabel(self.app_actualizar_usuarios, text="Contraseña", font=self.letra_label)
        label_password_admin.place(x=80, y=230, anchor="w")
        
        self.entry_password_admin = ctk.CTkEntry(self.app_actualizar_usuarios, width=170)
        self.entry_password_admin.place(x=80, y=270, anchor="w")
        
        #####################################################
        
        label_repeat_password_admin = ctk.CTkLabel(self.app_actualizar_usuarios, text="Repita su contraseña", font=self.letra_label)
        label_repeat_password_admin.place(x=80, y=310, anchor="w")
        
        self.entry_repeat_password_admin = ctk.CTkEntry(self.app_actualizar_usuarios, width=170)
        self.entry_repeat_password_admin.place(x=80, y=350, anchor="w")
        
        ######################################
        
        label_correo_admin = ctk.CTkLabel(self.app_actualizar_usuarios, text="Correo", font=self.letra_label)
        label_correo_admin.place(x=80, y=390, anchor="w")
        
        self.entry_correo_admin = ctk.CTkEntry(self.app_actualizar_usuarios, width=170)
        self.entry_correo_admin.place(x=80, y=430, anchor="w")
        
        ######################################
        
        label_id_admin = ctk.CTkLabel(self.app_actualizar_usuarios, text="ID-Usuario", font=self.letra_label)
        label_id_admin.place(x=80, y=470, anchor="w")
        
        self.entry_id_admin = ctk.CTkEntry(self.app_actualizar_usuarios, width=170)
        self.entry_id_admin.place(x=80, y=510, anchor="w")
        
        ######################################
        
        frame_divisor = ctk.CTkFrame(self.app_actualizar_usuarios, width=5, height=200)
        frame_divisor.place(x=325, y=270, anchor="w")
        
        ##################################################
        
        label_usuario_medico = ctk.CTkLabel(self.app_actualizar_usuarios, text="Usuario Médico", font=self.letra_label)
        label_usuario_medico.place(x=395, y=150, anchor="w")
        
        self.entry_usuario_medico = ctk.CTkEntry(self.app_actualizar_usuarios, width=170)
        self.entry_usuario_medico.place(x=395, y=190, anchor="w")
        
        #################################################
        
        label_passentry_password_medico = ctk.CTkLabel(self.app_actualizar_usuarios, text="Contraseña", font=self.letra_label)
        label_passentry_password_medico.place(x=395, y=230, anchor="w")
        
        self.entry_password_medico = ctk.CTkEntry(self.app_actualizar_usuarios, width=170)
        self.entry_password_medico.place(x=395, y=270, anchor="w")
        
        #################################################
        
        label_password_repeat_medico = ctk.CTkLabel(self.app_actualizar_usuarios, text="Repita su contraseña", font=self.letra_label)
        label_password_repeat_medico.place(x=395, y=310, anchor="w")
        
        self.entry_password_repeat_medico = ctk.CTkEntry(self.app_actualizar_usuarios, width=170)
        self.entry_password_repeat_medico.place(x=395, y=350, anchor="w")
        
        ################################################
        
        label_correo_medico = ctk.CTkLabel(self.app_actualizar_usuarios, text="Correo", font=self.letra_label)
        label_correo_medico.place(x=395, y=390, anchor="w")
        
        self.entry_correo_medico = ctk.CTkEntry(self.app_actualizar_usuarios, width=170)
        self.entry_correo_medico.place(x=395, y=430, anchor="w")
        
        ################################################
        
        label_id_medico = ctk.CTkLabel(self.app_actualizar_usuarios, text="ID-Usuario", font=self.letra_label)
        label_id_medico.place(x=395, y=470, anchor="w")
        
        self.entry_id_medico = ctk.CTkEntry(self.app_actualizar_usuarios, width=170)
        self.entry_id_medico.place(x=395, y=510, anchor="w")
        
        ################################################
        
        frame_divisor2 = ctk.CTkFrame(self.app_actualizar_usuarios, width=5, height=200)
        frame_divisor2.place(x=625, y=270, anchor="w")
        
        ###################################################
        
        label_usuario_asistente = ctk.CTkLabel(self.app_actualizar_usuarios, text="Usuario Asistente", font=self.letra_label)
        label_usuario_asistente.place(x=700, y=150, anchor="w")
        
        self.entry_usuario_asistente = ctk.CTkEntry(self.app_actualizar_usuarios, width=170)
        self.entry_usuario_asistente.place(x=700, y=190, anchor="w")
        
        #################################################
        
        label_password_asistente = ctk.CTkLabel(self.app_actualizar_usuarios, text="Contraseña", font=self.letra_label)
        label_password_asistente.place(x=700, y=230, anchor="w")
        
        self.entry_password_asistente = ctk.CTkEntry(self.app_actualizar_usuarios, width=170)
        self.entry_password_asistente.place(x=700, y=270, anchor="w")
        
        #################################################
        
        label_password_repeat_asistente = ctk.CTkLabel(self.app_actualizar_usuarios, text="Repita su contraseña", font=self.letra_label)
        label_password_repeat_asistente.place(x=700, y=310, anchor="w")
        
        self.entry_password_repeat_asistente = ctk.CTkEntry(self.app_actualizar_usuarios, width=170)
        self.entry_password_repeat_asistente.place(x=700, y=350, anchor="w")
        
        #################################################
        
        label_correo_asistente = ctk.CTkLabel(self.app_actualizar_usuarios, text="Correo", font=self.letra_label)
        label_correo_asistente.place(x=700, y=390, anchor="w")
        
        self.entry_correo_asistente = ctk.CTkEntry(self.app_actualizar_usuarios, width=170)
        self.entry_correo_asistente.place(x=700, y=430, anchor="w")
        
        ################################################
        
        label_id_asistente = ctk.CTkLabel(self.app_actualizar_usuarios, text="ID-Usuario", font=self.letra_label)
        label_id_asistente.place(x=700, y=470, anchor="w")
        
        self.entry_id_asistente = ctk.CTkEntry(self.app_actualizar_usuarios, width=170)
        self.entry_id_asistente.place(x=700, y=510, anchor="w")
        
        #################################################  
        
        
        button_ingresar_user_admin = ctk.CTkButton(self.app_actualizar_usuarios,
                                                 text="Actualizar",
                                                 width=12, 
                                                 command=self.actualizar_usuario_admin)
        button_ingresar_user_admin.place(x=80, y=600, anchor="w")
        
        button_consultar_admin = ctk.CTkButton(self.app_actualizar_usuarios, 
                                               text="Consultar",
                                               width=12,
        ...
        button_actualizar_user_medico = ctk.CTkButton(self.app_actualizar_usuarios,
                                                 text="Actualizar",
                                                 width=12, 
                                                 command=self.actualizar_usuario_medico)
        button_actualizar_user_medico.place(x=395, y=600, anchor="w")
        
        button_consultar_medico = ctk.CTkButton(self.app_actualizar_usuarios,
                                                 text="Consultar",
                                                 width=12, 
        ...
        button_actualizar_user_asistente = ctk.CTkButton(self.app_actualizar_usuarios,
                                                 text="Actualizar",
                                                 width=12, 
                                                 command=self.actualizar_usuario_asistente)
        button_actualizar_user_asistente.place(x=700, y=600, anchor="w")
        
        button_consultar_asistente = ctk.CTkButton(self.app_actualizar_usuarios,
                                                 text="Consultar",
                                                 width=12, 
                ...
                if password == password_repeat:
                    if len(password) > 5:
                        if len(correo) < 31:
                            ...
                            if dato is None:
                                verified = self.db.actualizar_admins(id_user=id_user, usuario=usuario_admin, password=password_repeat, correo=correo)
                                if verified == True:
                                        CTkMessagebox(title="Administración | Actualización de Usuarios",
                                        message="El usuario admin fue actualizado correctamente!",
                                        icon="check",
                                        option_1="Cerrar",
                                        fade_in_duration=35)
                                        self.limpiar_datos_admin()
    ...
    def actualizar_usuario_medico(self):
                ...
                if password_medico == password_repeat_medico:
                    if len(password_medico) > 5:
                        if len(correo_medico) < 31:
                            ...
                            if dato is None:
                                verified = self.db.actualizar_medicos(id_user=id_user, usuario=usuario_medico, password=password_repeat_medico, correo=correo_medico)
                                if verified == True:
                                        CTkMessagebox(title="Administración | Actualizar de Usuarios",
                                        message="El usuario médico fue actualizado correctamente!",
                                        icon="check",
                                        option_1="Cerrar",
                                        fade_in_duration=35)
                                        self.limpiar_datos_medico()
    ...
    def actualizar_usuario_asistente(self):
                ...
                if password_asistente == password_repeat_asistente:
                    if len(password_asistente) > 5:
                        if len(correo_asistente) < 31:
                            ...
                            if dato is None:
                                verified = self.db.actualizar_asistentes(id_user=id_user, usuario=usuario_asistente, password=password_asistente, correo=correo_asistente)
                                if verified == True:
                                        CTkMessagebox(title="Administración | Actualización de Usuarios",
                                        message="El usuario asistente fue actualizado correctamente!",
                                        icon="check",
                                        option_1="Cerrar",
                                        fade_in_duration=35)
                                        self.limpiar_datos_asistente()
